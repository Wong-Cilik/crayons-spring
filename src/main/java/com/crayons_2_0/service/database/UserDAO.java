...
public class UserDAO  {
     ...
     */
    public boolean updateUser(CrayonsUser user, String eMail, String firstName, String lastName, String password) {
        ...
        jdbcTemplate.update("UPDATE users SET email=?, password=?, firstname=?, lastname=? WHERE email=? ", eMail,
                password, firstName, lastName, user.getEmail());
        return true;
    }
    ...
    public void deleteUser(String user) {
        ...
        try {
            ...
            throw new UsernameNotFoundException("User with mail:" + user + "doesnt exists!");
        }
    }
    ...
    // "CREATE TABLE IF NOT EXISTS users(email VARCHAR(100) NOT NULL, password
    // VARCHAR(100) NOT NULL, firstname VARCHAR(100) NOT NULL,lastname
    // VARCHAR(100) NOT NULL, language VARCHAR(100) NOT NULL, permission int)");
    //
    // log.info("@@ > Done.");
    // }
    ...
    public boolean updateLanguage(CrayonsUser user, Language newLanguage) {
        jdbcTemplate.update("UPDATE users SET language=? WHERE email=?", newLanguage.toString(), user.getEmail());
        return true;
    }
}
