...
public class UnitUserView extends VerticalLayout implements View {
	...
	void init() {
		...
		image.setSource(new FileResource(
				new File(
						"/src/main/resources/com/crayons_2_0/images/architecture-detailed-hi.png")));
		image.setWidth(400, Unit.PIXELS);
		addImageArea("Vaadin detailed architecture", image);
		addMultipleChoise("How old are you?", Arrays.asList("10", "20", "30"),
				"20");
		addMultipleChoise("What's your name?",
				Arrays.asList("Natalia", "Julius", "Marc"), "Natalia");
	...
	public void addMultipleChoise(String questionText,
			List<String> answersList, String rightAnswer) {
		page.addComponent(buildMultipleChoise(questionText, answersList,
				rightAnswer));
	}

	private Component buildMultipleChoise(String questionText,
			List<String> answersList, String rightAnswer) {
		VerticalLayout layout = new VerticalLayout();
		layout.setSpacing(true);
		...
		answers.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				responces.put(questionText,
						((String) answers.getValue()).equals(rightAnswer));
			}
		});
		 ...
		 * responces.put(questionText, ((String)
		 * answers.getValue()).equals(rightAnswer)); }
		 * 
		 * });
		 */
