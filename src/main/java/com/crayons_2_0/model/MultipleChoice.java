...
public class MultipleChoice extends VerticalLayout {
	...
	public MultipleChoice() {
		...
		OptionGroup sample = new OptionGroup(
				lang.getString("MultipleChoiceExample"));
		for (int i = 0; i < 5; i++) {
			sample.addItem(i);
		...
		final CheckBox disableOptionsCheckBox = new CheckBox(
				lang.getString("SomeOptionsDisabled"));
		...
		final CheckBox htmlCaptionsCheckBox = new CheckBox(
				lang.getString("CaptionsWithHTMLContent"));
		...
		/*
		 * TODO sample.addValueChangeListener(e ->
		 * Notification.show("Value changed:",
		 * String.valueOf(e.getProperty().getValue()), Type.class));
		 */
		addComponent(sample);
