...
public class GraphTest {
	...
	public void setUp() throws Exception {
		...
		dummyUser = new CrayonsUser("first", "last", "dummy@web.de", "123456",
				"German", 2, true, true, false, false, authorities);
		...
		contentThree = new UnitNode("three", testGraph.getStartUnit(),
				testGraph);
	...
	public void testGetEdgeSequence() {
		assertTrue(testGraph.addUnit(contentOne, testGraph.getStartUnit(),
				contentTwo));
		assertTrue(testGraph.addUnit(contentTwo, contentOne, contentThree));
		assertTrue(testGraph.addUnit(contentThree, contentTwo,
				testGraph.getEndUnit()));
	...
	public void testGetNodeByName() {
		...
		testGraph.addUnit(contentThree, contentTwo, testGraph.getEndUnit());
		assertEquals("one",
				testGraph.getNodeByName(contentOne.getUnitNodeTitle()));
		assertEquals(contentOne.getUnitNodeTitle(),
				testGraph.getNodeByName(contentOne.getUnitNodeTitle()));
		assertEquals("two",
				testGraph.getNodeByName(contentTwo.getUnitNodeTitle()));
		assertEquals(contentTwo.getUnitNodeTitle(),
				testGraph.getNodeByName(contentTwo.getUnitNodeTitle()));
		assertEquals("three",
				testGraph.getNodeByName(contentThree.getUnitNodeTitle()));
		assertEquals(contentThree.getUnitNodeTitle(),
				testGraph.getNodeByName(contentThree.getUnitNodeTitle()));
		assertEquals("End", testGraph.getNodeByName(testGraph.getEndUnit()
				.getUnitNodeTitle()));
		assertEquals(testGraph.getEndUnit().getUnitNodeTitle(), testGraph
				.getEndUnit().getUnitNodeTitle());
		assertEquals("Start", testGraph.getNodeByName(testGraph.getStartUnit()
				.getUnitNodeTitle()));
		assertEquals(testGraph.getStartUnit().getUnitNodeTitle(), testGraph
				.getStartUnit().getUnitNodeTitle());
	}
