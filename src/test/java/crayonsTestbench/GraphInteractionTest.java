...
public class GraphInteractionTest extends TestBenchTestCase {
    ...
    public void navigateToGrapheditor() {
        ...
        $(TextFieldElement.class).caption(lang.getString("Login") + ": ").first().setValue("client@web.de");

        // 2. Enter "123456" into the password field
        $(PasswordFieldElement.class).caption(lang.getString("Password") + ": ").first().setValue("123456");
        ...
        $(TabSheetElement.class).$(TextFieldElement.class).first().setValue("GraphTest");
        ...
        List<String> captions = $(TabSheetElement.class).first().getTabCaptions();
        captions.remove(0);
        ...
        // 9. Click "Grapheditor"
        $(ButtonElement.class).caption(lang.getString("GraphEditor")).first().click();

        // 10. Click "Add unit"
        $(ButtonElement.class).caption("Add unit").first().click();

        // 11. Click "Add unit"
        $(TextFieldElement.class).caption("Unit title").first().setValue("TestUnit");

        // 12. Select Start Unit
        $(ComboBoxElement.class).caption("Select the previous unit").first().selectByText("Start");

        // 13. Select End Unit
        $(ComboBoxElement.class).caption("Select the next unit").first().selectByText("End");

        // 14. Click "Add unit"
        $(TextFieldElement.class).caption("Unit title").first().setValue("TestUnit2");

        // 15. Select Start Unit
        $(ComboBoxElement.class).caption("Select the previous unit").first().selectByText("TestUnit");
